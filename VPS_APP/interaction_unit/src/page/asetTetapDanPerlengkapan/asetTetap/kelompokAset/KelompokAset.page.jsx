...
import KelompokAsetForm from "./component/KelompokAsetForm"
import { showAlert, showDialog, showError } from "../../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { KelompokAsetPrint } from "./component/KelompokAsetPrint"
...
const KelompokAsetPage = () => {
    ...
    const [addKelompokAset, setAddKelompokAset] = useState(false)
    const [kelompokAsetEdit, setKelompokAsetEdit] = useState({
        name: ""
    })
    ...
    const KelompokAsetPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => KelompokAsetPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiKelompokAsetCRUD
            ...
            .then(resData => {
                ...
                setKelompokAset(resData?.data?.entry)
    ...
    const _editKelompokAset = (i) => {
        let kelompokAsetSelected = kelompokAset[i]
        setKelompokAsetEdit(kelompokAsetSelected)
        setAddKelompokAset(!addKelompokAset)
    }

    const _deleteKelompokAset = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let kelompokAsetSelected = kelompokAset[i]
            apiKelompokAsetCRUD
                .custom(`/${kelompokAsetSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Kelompok Aset" />
            {
                addKelompokAset ?
                    <KelompokAsetForm
                        setAddKelompokAsetEvent={() => setAddKelompokAset(false)}
                        getData={_getData}
                        kelompokAsetEdit={kelompokAsetEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                <button className="btn btn-sm bg-blue-900 text-white border-none"
                                    onClick={() => {
                                        setKelompokAsetEdit(null)
                                        setAddKelompokAset(!addKelompokAset)
                                    }}
                                ><FaPlus /> Tambah Kelompok Aset</button>
                                <div className="hidden">
                                    <KelompokAsetPrint
                                        data={kelompokAset}
                                        ref={KelompokAsetPrintRef}
                            ...
                            <table className="table">
                                ...
                                <thead>
                                    <tr className="sticky top-0 bg-white py-4 text-black">
                                        ...
                                        <th>Masa Penyusutan Aset</th>
                                        <th>Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {
                                        kelompokAset?.map((item, i) => {
                                            return <>
                                                <tr key={i}>
                                                    <td>{i + 1}.</td>
                                                    <td>{item.name}</td>
                                                    <td>{item.masa_penyusutan} Tahun</td>
...
export default KelompokAsetPage