...
import KategoriPerlengkapanForm from "./component/KategoriPerlengkapanForm"
import { showAlert, showDialog, showError } from "../../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { KategoriPerlengkapanPrint } from "./component/KategoriPerlengkapanPrint"
import { getBulanByIndex } from "../../../../helper/date.helper"

const KategoriPerlengkapanPage = () => {

    const dataContext = useDataContext()
    const { data } = dataContext

    const [kategoriPerlengkapan, setKategoriPerlengkapan] = useState([])
    const [isLoading, setIsLoading] = useState(false)
    const [addKategoriPerlengkapan, setAddKategoriPerlengkapan] = useState(false)
    const [kategoriPerlengkapanEdit, setKategoriPerlengkapanEdit] = useState({
        name: ""
    })
    ...
    const KategoriPerlengkapanPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => KategoriPerlengkapanPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiKategoriPerlengkapanCRUD
            ...
            .then(resData => {
                ...
                setKategoriPerlengkapan(resData?.data?.entry)
    ...
    const _editKategoriPerlengkapan = (i) => {
        let kategoriPerlengkapanSelected = kategoriPerlengkapan[i]
        setKategoriPerlengkapanEdit(kategoriPerlengkapanSelected)
        setAddKategoriPerlengkapan(!addKategoriPerlengkapan)
    }

    const _deleteKategoriPerlengkapan = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let kategoriPerlengkapanSelected = kategoriPerlengkapan[i]
            apiKategoriPerlengkapanCRUD
                .custom(`/${kategoriPerlengkapanSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Kategori Perlengkapan" />
            {
                addKategoriPerlengkapan ?
                    <KategoriPerlengkapanForm
                        setAddKategoriPerlengkapanEvent={() => setAddKategoriPerlengkapan(false)}
                        getData={_getData}
                        kategoriPerlengkapanEdit={kategoriPerlengkapanEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                <button className="btn btn-sm bg-blue-900 text-white border-none"
                                    onClick={() => {
                                        setKategoriPerlengkapanEdit(null)
                                        setAddKategoriPerlengkapan(!addKategoriPerlengkapan)
                                    }}
                                ><FaPlus /> Tambah Kategori Perlengkapan</button>
                                <div className="hidden">
                                    <KategoriPerlengkapanPrint
                                        data={kategoriPerlengkapan}
                                        ref={KategoriPerlengkapanPrintRef}
                                ...
                                >
                                    <FaPrint /> Cetak Kategori Perlengkapan
                                </button>
                            </div>
                        </div>
                        <div className="overflow-x-auto bg-white shadow-xl rounded-md h-[50vh] no-scrollbar px-6 pb-4">
                            <table className="table">
                                ...
                                <tbody>
                                    {
                                        kategoriPerlengkapan?.map((item, i) => {
                                            return <>
                                                <tr key={i}>
                                                    <td>{i + 1}.</td>
                                                    <td>{item.name}</td>
                                                    ...
                                                    <td className="flex gap-x-2">
                                                        <FaPen size={12} className="text-yellow-500 cursor-pointer"
                                                            onClick={() => {
                                                                _editKategoriPerlengkapan(i)
                                                            }} />
                                                        <FaTrash size={12} className="text-red-500 cursor-pointer"
                                                            onClick={() => {
                                                                _deleteKategoriPerlengkapan(i)
                                                            }}
                                                        />
                                                    </td>
                                                </tr>
...
export default KategoriPerlengkapanPage