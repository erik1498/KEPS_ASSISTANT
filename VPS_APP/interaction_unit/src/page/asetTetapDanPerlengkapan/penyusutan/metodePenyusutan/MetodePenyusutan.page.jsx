...
import MetodePenyusutanForm from "./component/MetodePenyusutanForm"
import { showAlert, showDialog, showError } from "../../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { MetodePenyusutanPrint } from "./component/MetodePenyusutanPrint"
import { getBulanByIndex } from "../../../../helper/date.helper"

const MetodePenyusutanPage = () => {
    ...
    const [addMetodePenyusutan, setAddMetodePenyusutan] = useState(false)
    const [metodePenyusutanEdit, setMetodePenyusutanEdit] = useState({
        name: ""
    })
    ...
    const MetodePenyusutanPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => MetodePenyusutanPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiMetodePenyusutanCRUD
            ...
            .then(resData => {
                ...
                setMetodePenyusutan(resData?.data?.entry)
    ...
    const _editMetodePenyusutan = (i) => {
        let metodePenyusutanSelected = metodePenyusutan[i]
        setMetodePenyusutanEdit(metodePenyusutanSelected)
        setAddMetodePenyusutan(!addMetodePenyusutan)
    }

    const _deleteMetodePenyusutan = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let metodePenyusutanSelected = metodePenyusutan[i]
            apiMetodePenyusutanCRUD
                .custom(`/${metodePenyusutanSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Metode Penyusutan" />
            {
                addMetodePenyusutan ?
                    <MetodePenyusutanForm
                        setAddMetodePenyusutanEvent={() => setAddMetodePenyusutan(false)}
                        getData={_getData}
                        metodePenyusutanEdit={metodePenyusutanEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                ...
                                >
                                    <FaPrint /> Cetak Metode Penyusutan
                                </button>
                            </div>
                        </div>
...
export default MetodePenyusutanPage