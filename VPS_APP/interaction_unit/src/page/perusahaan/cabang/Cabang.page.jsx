...
import CabangForm from "./component/CabangForm"
import { showAlert, showDialog, showError } from "../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { CabangPrint } from "./component/CabangPrint"
...
const CabangPage = () => {
    ...
    const [addCabang, setAddCabang] = useState(false)
    const [cabangEdit, setCabangEdit] = useState({
        name: "",
    })
    ...
    const CabangPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => CabangPrintRef.current,
    });
    ...
    const _editCabang = (i) => {
        let cabangSelected = cabang[i]
        setCabangEdit(cabangSelected)
        setAddCabang(!addCabang)
    }

    const _deleteCabang = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let cabangSelected = cabang[i]
            apiCabangCRUD
                .custom(`/${cabangSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Cabang" />
            {
                addCabang ?
                    <CabangForm
                        setAddCabangEvent={() => setAddCabang(false)}
                        getData={_getData}
                        cabangEdit={cabangEdit}
                    />
                    :
                    <>
