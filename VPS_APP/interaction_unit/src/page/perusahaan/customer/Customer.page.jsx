...
import { CustomerPrint } from "./component/CustomerPrint"
...
const CustomerPage = () => {
    ...
    const _getData = (searchParam = "") => {
        ...
        apiCustomerCRUD
            ...
            .then(resData => {
                ...
                setCustomer(resData?.data?.entry)
    ...
    const _editCustomer = (i) => {
        let customerSelected = customer[i]
        setCustomerEdit(customerSelected)
        setAddCustomer(!addCustomer)
    }

    const _deleteCustomer = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let customerSelected = customer[i]
            apiCustomerCRUD
                .custom(`/${customerSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        ...
        <div>
            <PageTitle title="Customer" />
            {
                addCustomer ?
                    <CustomerForm
                        setAddCustomerEvent={() => setAddCustomer(false)}
                        getData={_getData}
