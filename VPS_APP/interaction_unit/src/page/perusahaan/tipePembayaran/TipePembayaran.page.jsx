...
import TipePembayaranForm from "./component/TipePembayaranForm"
import { showAlert, showDialog, showError } from "../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { TipePembayaranPrint } from "./component/TipePembayaranPrint"
import { getBulanByIndex } from "../../../helper/date.helper"

const TipePembayaranPage = () => {
    ...
    const [addTipePembayaran, setAddTipePembayaran] = useState(false)
    const [tipePembayaranEdit, setTipePembayaranEdit] = useState({
        name: ""
    })
    ...
    const TipePembayaranPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => TipePembayaranPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiTipePembayaranCRUD
            ...
            .then(resData => {
                ...
                setTipePembayaran(resData?.data?.entry)
    ...
    const _editTipePembayaran = (i) => {
        let tipePembayaranSelected = tipePembayaran[i]
        setTipePembayaranEdit(tipePembayaranSelected)
        setAddTipePembayaran(!addTipePembayaran)
    }

    const _deleteTipePembayaran = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let tipePembayaranSelected = tipePembayaran[i]
            apiTipePembayaranCRUD
                .custom(`/${tipePembayaranSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        ...
        <div>
            <PageTitle title="TipePembayaran" />
            {
                addTipePembayaran ?
                    <TipePembayaranForm
                        setAddTipePembayaranEvent={() => setAddTipePembayaran(false)}
                        getData={_getData}
                        tipePembayaranEdit={tipePembayaranEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                <button className="btn btn-sm bg-blue-900 text-white border-none"
                                    onClick={() => {
                                        setTipePembayaranEdit(null)
                                        setAddTipePembayaran(!addTipePembayaran)
                                    }}
                                ><FaPlus /> Tambah Tipe Pembayaran</button>
                                <div className="hidden">
                                    <TipePembayaranPrint
                                        data={tipePembayaran}
                                        ref={TipePembayaranPrintRef}
...
export default TipePembayaranPage