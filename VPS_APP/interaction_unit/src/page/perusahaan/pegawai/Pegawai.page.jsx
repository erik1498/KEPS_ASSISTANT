...
import { PegawaiPrint } from "./component/PegawaiPrint"
...
const PegawaiPage = () => {
    ...
    const [addPegawai, setAddPegawai] = useState(false)
    const [pegawaiEdit, setPegawaiEdit] = useState({
        name: "",
    })
    ...
    const PegawaiPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => PegawaiPrintRef.current,
    });
    ...
    const _editPegawai = (i) => {
        let pegawaiSelected = pegawai[i]
        setPegawaiEdit(pegawaiSelected)
        setAddPegawai(!addPegawai)
    }

    const _deletePegawai = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let pegawaiSelected = pegawai[i]
            apiPegawaiCRUD
                .custom(`/${pegawaiSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Pegawai" />
            {
                addPegawai ?
                    <PegawaiForm
                        setAddPegawaiEvent={() => setAddPegawai(false)}
                        getData={_getData}
