...
import { JabatanPrint } from "./component/JabatanPrint"
...
const JabatanPage = () => {
    ...
    const [addJabatan, setAddJabatan] = useState(false)
    ...
    const JabatanPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => JabatanPrintRef.current,
    });
    ...
    const _editJabatan = (i) => {
        let jabatanSelected = jabatan[i]
        setJabatanEdit(jabatanSelected)
        setAddJabatan(!addJabatan)
    }

    const _deleteJabatan = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let jabatanSelected = jabatan[i]
            apiJabatanCRUD
                .custom(`/${jabatanSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Jabatan" />
            {
                addJabatan ?
                    <JabatanForm
                        setAddJabatanEvent={() => setAddJabatan(false)}
                        getData={_getData}
                        jabatanEdit={jabatanEdit}
                    />
                    :
                    <>
