...
import DivisiForm from "./component/DivisiForm"
import { showAlert, showDialog, showError } from "../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { DivisiPrint } from "./component/DivisiPrint"
...
const DivisiPage = () => {
    ...
    const [divisiEdit, setDivisiEdit] = useState({
        name: ""
    })
    ...
    const DivisiPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => DivisiPrintRef.current,
    });
    ...
    const _editDivisi = (i) => {
        let divisiSelected = divisi[i]
        setDivisiEdit(divisiSelected)
        setAddDivisi(!addDivisi)
    }

    const _deleteDivisi = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let divisiSelected = divisi[i]
            apiDivisiCRUD
                .custom(`/${divisiSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Divisi" />
            {
                addDivisi ?
                    <DivisiForm
                        setAddDivisiEvent={() => setAddDivisi(false)}
                        getData={_getData}
                        divisiEdit={divisiEdit}
                    />
                    :
                    <>
