...
import { apiDaftarBahanBakuCRUD } from "../../../../service/endPointList.api"
import { useEffect } from "react"
import Pagination from "../../../../component/general/Pagination"
import DaftarBahanBakuForm from "./component/DaftarBahanBakuForm"
import { showAlert, showDialog, showError } from "../../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { DaftarBahanBakuPrint } from "./component/DaftarBahanBakuPrint"
import { getBulanByIndex } from "../../../../helper/date.helper"

const DaftarBahanBakuPage = () => {

    const dataContext = useDataContext()
    const { data } = dataContext

    const [daftarBahanBaku, setDaftarBahanBaku] = useState([])
    const [isLoading, setIsLoading] = useState(false)
    const [addDaftarBahanBaku, setAddDaftarBahanBaku] = useState(false)
    const [daftarBahanBakuEdit, setDaftarBahanBakuEdit] = useState({
        name: ""
    })
    ...
    const DaftarBahanBakuPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => DaftarBahanBakuPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiDaftarBahanBakuCRUD
            ...
            .then(resData => {
                ...
                setDaftarBahanBaku(resData?.data?.entry)
    ...
    const _editDaftarBahanBaku = (i) => {
        let daftarBahanBakuSelected = daftarBahanBaku[i]
        setDaftarBahanBakuEdit(daftarBahanBakuSelected)
        setAddDaftarBahanBaku(!addDaftarBahanBaku)
    }

    const _deleteDaftarBahanBaku = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let daftarBahanBakuSelected = daftarBahanBaku[i]
            apiDaftarBahanBakuCRUD
                .custom(`/${daftarBahanBakuSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Daftar Bahan Baku" />
            {
                addDaftarBahanBaku ?
                    <DaftarBahanBakuForm
                        setAddDaftarBahanBakuEvent={() => setAddDaftarBahanBaku(false)}
                        getData={_getData}
                        daftarBahanBakuEdit={daftarBahanBakuEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                <button className="btn btn-sm bg-blue-900 text-white border-none"
                                    onClick={() => {
                                        setDaftarBahanBakuEdit(null)
                                        setAddDaftarBahanBaku(!addDaftarBahanBaku)
                                    }}
                                ><FaPlus /> Tambah Daftar Bahan Baku</button>
                                <div className="hidden">
                                    <DaftarBahanBakuPrint
                                        data={daftarBahanBaku}
                                        ref={DaftarBahanBakuPrintRef}
                                ...
                                >
                                    <FaPrint /> Cetak Daftar Bahan Baku
                                </button>
                            </div>
                        </div>
                            ...
                            <table className="table">
                                ...
                                <thead>
                                    <tr className="sticky top-0 bg-white py-4 text-black">
                                        <th width={12}>No</th>
                                        <th>Nama Daftar Bahan Baku</th>
                                        <th>Kategori Bahan Baku</th>
                                        <th>Jenis Bahan Baku</th>
                                        <th>Jenis Penjualan Bahan Baku</th>
                                        <th>PPN</th>
                                        <th>Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {
                                        daftarBahanBaku?.map((item, i) => {
                                            return <>
                                                <tr key={i}>
                                                    <td>{i + 1}.</td>
                                                    <td>{item.name}</td>
                                                    <td>{item.kategori_bahan_baku_name}</td>
                                                    <td>{item.jenis_bahan_baku_name}</td>
                                                    <td>{item.jenis_penjualan_bahan_baku_name}</td>
                                                    <td>{item.ppn == 1 ? "Ya" : "Tidak"}</td>
                                                    <td className="flex gap-x-2">
                                                        <FaPen size={12} className="text-yellow-500 cursor-pointer"
                                                            onClick={() => {
                                                                _editDaftarBahanBaku(i)
                                                            }} />
                                                        <FaTrash size={12} className="text-red-500 cursor-pointer"
                                                            onClick={() => {
                                                                _deleteDaftarBahanBaku(i)
                                                            }}
                                                        />
                                                    </td>
                                                </tr>
...
export default DaftarBahanBakuPage