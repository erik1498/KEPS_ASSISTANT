...
import JenisBahanBakuForm from "./component/JenisBahanBakuForm"
import { showAlert, showDialog, showError } from "../../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { JenisBahanBakuPrint } from "./component/JenisBahanBakuPrint"
import { getBulanByIndex } from "../../../../helper/date.helper"

const JenisBahanBakuPage = () => {

    const dataContext = useDataContext()
    const { data } = dataContext

    const [jenisBahanBaku, setJenisBahanBaku] = useState([])
    const [isLoading, setIsLoading] = useState(false)
    const [addJenisBahanBaku, setAddJenisBahanBaku] = useState(false)
    const [jenisBahanBakuEdit, setJenisBahanBakuEdit] = useState({
        name: ""
    })
    ...
    const JenisBahanBakuPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => JenisBahanBakuPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiJenisBahanBakuCRUD
            ...
            .then(resData => {
                ...
                setJenisBahanBaku(resData?.data?.entry)
    ...
    const _editJenisBahanBaku = (i) => {
        let jenisBahanBakuSelected = jenisBahanBaku[i]
        setJenisBahanBakuEdit(jenisBahanBakuSelected)
        setAddJenisBahanBaku(!addJenisBahanBaku)
    }

    const _deleteJenisBahanBaku = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let jenisBahanBakuSelected = jenisBahanBaku[i]
            apiJenisBahanBakuCRUD
                .custom(`/${jenisBahanBakuSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Jenis Bahan Baku" />
            {
                addJenisBahanBaku ?
                    <JenisBahanBakuForm
                        setAddJenisBahanBakuEvent={() => setAddJenisBahanBaku(false)}
                        getData={_getData}
                        jenisBahanBakuEdit={jenisBahanBakuEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                <button className="btn btn-sm bg-blue-900 text-white border-none"
                                    onClick={() => {
                                        setJenisBahanBakuEdit(null)
                                        setAddJenisBahanBaku(!addJenisBahanBaku)
                                    }}
                                ><FaPlus /> Tambah Jenis Bahan Baku</button>
                                <div className="hidden">
                                    <JenisBahanBakuPrint
                                        data={jenisBahanBaku}
                                        ref={JenisBahanBakuPrintRef}
                                ...
                                >
                                    <FaPrint /> Cetak Jenis Bahan Baku
                                </button>
                            </div>
                        </div>
...
export default JenisBahanBakuPage