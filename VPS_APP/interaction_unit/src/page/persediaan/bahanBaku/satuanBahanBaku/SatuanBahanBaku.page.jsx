...
import SatuanBahanBakuForm from "./component/SatuanBahanBakuForm"
import { showAlert, showDialog, showError } from "../../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { SatuanBahanBakuPrint } from "./component/SatuanBahanBakuPrint"
import { getBulanByIndex } from "../../../../helper/date.helper"

const SatuanBahanBakuPage = () => {
    ...
    const [addSatuanBahanBaku, setAddSatuanBahanBaku] = useState(false)
    const [satuanBahanBakuEdit, setSatuanBahanBakuEdit] = useState({
        name: ""
    })
    ...
    const SatuanBahanBakuPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => SatuanBahanBakuPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiSatuanBahanBakuCRUD
            ...
            .then(resData => {
                ...
                setSatuanBahanBaku(resData?.data?.entry)
    ...
    const _editSatuanBahanBaku = (i) => {
        let satuanBahanBakuSelected = satuanBahanBaku[i]
        setSatuanBahanBakuEdit(satuanBahanBakuSelected)
        setAddSatuanBahanBaku(!addSatuanBahanBaku)
    }

    const _deleteSatuanBahanBaku = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let satuanBahanBakuSelected = satuanBahanBaku[i]
            apiSatuanBahanBakuCRUD
                .custom(`/${satuanBahanBakuSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Satuan Bahan Baku" />
            {
                addSatuanBahanBaku ?
                    <SatuanBahanBakuForm
                        setAddSatuanBahanBakuEvent={() => setAddSatuanBahanBaku(false)}
                        getData={_getData}
                        satuanBahanBakuEdit={satuanBahanBakuEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                <button className="btn btn-sm bg-blue-900 text-white border-none"
                                    onClick={() => {
                                        setSatuanBahanBakuEdit(null)
                                        setAddSatuanBahanBaku(!addSatuanBahanBaku)
                                    }}
                                ><FaPlus /> Tambah Satuan Bahan Baku</button>
                                <div className="hidden">
                                    <SatuanBahanBakuPrint
                                        data={satuanBahanBaku}
                                        ref={SatuanBahanBakuPrintRef}
                                ...
                                >
                                    <FaPrint /> Cetak Satuan Bahan Baku
                                </button>
                            </div>
                        </div>
...
export default SatuanBahanBakuPage