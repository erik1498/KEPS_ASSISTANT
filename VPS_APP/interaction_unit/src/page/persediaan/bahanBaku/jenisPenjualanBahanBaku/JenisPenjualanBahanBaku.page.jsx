...
import JenisPenjualanBahanBakuForm from "./component/JenisPenjualanBahanBakuForm"
import { showAlert, showDialog, showError } from "../../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { JenisPenjualanBahanBakuPrint } from "./component/JenisPenjualanBahanBakuPrint"
import { getBulanByIndex } from "../../../../helper/date.helper"

const JenisPenjualanBahanBakuPage = () => {

    const dataContext = useDataContext()
    const { data } = dataContext

    const [jenisPenjualanBahanBaku, setJenisPenjualanBahanBaku] = useState([])
    const [isLoading, setIsLoading] = useState(false)
    const [addJenisPenjualanBahanBaku, setAddJenisPenjualanBahanBaku] = useState(false)
    const [jenisPenjualanBahanBakuEdit, setJenisPenjualanBahanBakuEdit] = useState({
        name: ""
    })
    ...
    const JenisPenjualanBahanBakuPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => JenisPenjualanBahanBakuPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiJenisPenjualanBahanBakuCRUD
            ...
            .then(resData => {
                ...
                setJenisPenjualanBahanBaku(resData?.data?.entry)
    ...
    const _editJenisPenjualanBahanBaku = (i) => {
        let jenisPenjualanBahanBakuSelected = jenisPenjualanBahanBaku[i]
        setJenisPenjualanBahanBakuEdit(jenisPenjualanBahanBakuSelected)
        setAddJenisPenjualanBahanBaku(!addJenisPenjualanBahanBaku)
    }

    const _deleteJenisPenjualanBahanBaku = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let jenisPenjualanBahanBakuSelected = jenisPenjualanBahanBaku[i]
            apiJenisPenjualanBahanBakuCRUD
                .custom(`/${jenisPenjualanBahanBakuSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Jenis Penjualan Bahan Baku" />
            {
                addJenisPenjualanBahanBaku ?
                    <JenisPenjualanBahanBakuForm
                        setAddJenisPenjualanBahanBakuEvent={() => setAddJenisPenjualanBahanBaku(false)}
                        getData={_getData}
                        jenisPenjualanBahanBakuEdit={jenisPenjualanBahanBakuEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                <button className="btn btn-sm bg-blue-900 text-white border-none"
                                    onClick={() => {
                                        setJenisPenjualanBahanBakuEdit(null)
                                        setAddJenisPenjualanBahanBaku(!addJenisPenjualanBahanBaku)
                                    }}
                                ><FaPlus /> Tambah Jenis Penjualan Bahan Baku</button>
                                <div className="hidden">
                                    <JenisPenjualanBahanBakuPrint
                                        data={jenisPenjualanBahanBaku}
                                        ref={JenisPenjualanBahanBakuPrintRef}
                                ...
                                >
                                    <FaPrint /> Cetak Jenis Penjualan Bahan Baku
                                </button>
                            </div>
                        </div>
                        <div className="overflow-x-auto bg-white shadow-xl rounded-md h-[50vh] no-scrollbar px-6 pb-4">
                            <table className="table">
                                ...
                                <tbody>
                                    {
                                        jenisPenjualanBahanBaku?.map((item, i) => {
                                            return <>
                                                <tr key={i}>
                                                    <td>{i + 1}.</td>
                                                    <td>{item.name}</td>
                                                    ...
                                                    <td className="flex gap-x-2">
                                                        <FaPen size={12} className="text-yellow-500 cursor-pointer"
                                                            onClick={() => {
                                                                _editJenisPenjualanBahanBaku(i)
                                                            }} />
                                                        <FaTrash size={12} className="text-red-500 cursor-pointer"
                                                            onClick={() => {
                                                                _deleteJenisPenjualanBahanBaku(i)
                                                            }}
                                                        />
                                                    </td>
                                                </tr>
...
export default JenisPenjualanBahanBakuPage