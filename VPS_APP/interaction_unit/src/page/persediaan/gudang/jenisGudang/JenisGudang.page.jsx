...
import JenisGudangForm from "./component/JenisGudangForm"
import { showAlert, showDialog, showError } from "../../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { JenisGudangPrint } from "./component/JenisGudangPrint"
...
const JenisGudangPage = () => {
    ...
    const [addJenisGudang, setAddJenisGudang] = useState(false)
    const [jenisGudangEdit, setJenisGudangEdit] = useState({
        name: ""
    })
    ...
    const JenisGudangPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => JenisGudangPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiJenisGudangCRUD
            ...
            .then(resData => {
                ...
                setJenisGudang(resData?.data?.entry)
    ...
    const _editJenisGudang = (i) => {
        let jenisGudangSelected = jenisGudang[i]
        setJenisGudangEdit(jenisGudangSelected)
        setAddJenisGudang(!addJenisGudang)
    }

    const _deleteJenisGudang = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let jenisGudangSelected = jenisGudang[i]
            apiJenisGudangCRUD
                .custom(`/${jenisGudangSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Jenis Gudang" />
            {
                addJenisGudang ?
                    <JenisGudangForm
                        setAddJenisGudangEvent={() => setAddJenisGudang(false)}
                        getData={_getData}
                        jenisGudangEdit={jenisGudangEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                <button className="btn btn-sm bg-blue-900 text-white border-none"
                                    onClick={() => {
                                        setJenisGudangEdit(null)
                                        setAddJenisGudang(!addJenisGudang)
                                    }}
                                ><FaPlus /> Tambah Jenis Gudang</button>
                                <div className="hidden">
                                    <JenisGudangPrint
                                        data={jenisGudang}
                                        ref={JenisGudangPrintRef}
                            ...
                            <table className="table">
                                ...
                                <thead>
                                    <tr className="sticky top-0 bg-white py-4 text-black">
                                        ...
                                        <th>Kode Jenis Gudang</th>
                                        <th>Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {
                                        jenisGudang?.map((item, i) => {
                                            return <>
                                                <tr key={i}>
                                                    <td>{i + 1}.</td>
                                                    <td>{item.name}</td>
