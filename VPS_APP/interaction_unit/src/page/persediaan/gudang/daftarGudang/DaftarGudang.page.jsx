...
import DaftarGudangForm from "./component/DaftarGudangForm"
import { showAlert, showDialog, showError } from "../../../../helper/form.helper"
import { useRef } from "react"
import { useReactToPrint } from "react-to-print"
import { DaftarGudangPrint } from "./component/DaftarGudangPrint"
import { getBulanByIndex } from "../../../../helper/date.helper"

const DaftarGudangPage = () => {
    ...
    const [addDaftarGudang, setAddDaftarGudang] = useState(false)
    const [daftarGudangEdit, setDaftarGudangEdit] = useState({
        name: ""
    })
    ...
    const DaftarGudangPrintRef = useRef();
    const handlePrint = useReactToPrint({
        content: () => DaftarGudangPrintRef.current,
    });
    ...
    const _getData = (searchParam = "") => {
        ...
        apiDaftarGudangCRUD
            ...
            .then(resData => {
                ...
                setDaftarGudang(resData?.data?.entry)
    ...
    const _editDaftarGudang = (i) => {
        let daftarGudangSelected = daftarGudang[i]
        setDaftarGudangEdit(daftarGudangSelected)
        setAddDaftarGudang(!addDaftarGudang)
    }

    const _deleteDaftarGudang = async (i) => {
        if (await showDialog("Hapus", "Yakin ingin hapus data ini ?")) {
            let daftarGudangSelected = daftarGudang[i]
            apiDaftarGudangCRUD
                .custom(`/${daftarGudangSelected.uuid}`, "DELETE")
                .then(() => {
    ...
    return <Wrap
        isLoading={isLoading}>
        <div>
            <PageTitle title="Daftar Gudang" />
            {
                addDaftarGudang ?
                    <DaftarGudangForm
                        setAddDaftarGudangEvent={() => setAddDaftarGudang(false)}
                        getData={_getData}
                        daftarGudangEdit={daftarGudangEdit}
                    />
                    :
                    <>
                        <div className="bg-white py-3 px-6 mb-3 rounded-md flex justify-between shadow-2xl">
                            ...
                            <div className="flex gap-x-2 items-center">
                                <button className="btn btn-sm bg-blue-900 text-white border-none"
                                    onClick={() => {
                                        setDaftarGudangEdit(null)
                                        setAddDaftarGudang(!addDaftarGudang)
                                    }}
                                ><FaPlus /> Tambah Daftar Gudang</button>
                                <div className="hidden">
                                    <DaftarGudangPrint
                                        data={daftarGudang}
                                        ref={DaftarGudangPrintRef}
                        ...
                        <div className="overflow-x-auto bg-white shadow-xl rounded-md h-[50vh] no-scrollbar px-6 pb-4">
                            <table className="table">
                                ...
                                <tbody>
                                    {
                                        daftarGudang?.map((item, i) => {
                                            return <>
                                                <tr key={i}>
                                                    <td>{i + 1}.</td>
                                                    <td>{item.name}</td>
                                                    <td>{item.kategori_gudang_name}</td>
                                                    <td>{item.jenis_gudang_name}</td>
...
export default DaftarGudangPage