                                    ...
                                    aktivitasDokumenList.map((item, i) => {
                                        return <div className="mt-4 border-b-2 pb-4 border-gray-500">
                                            <div className="flex justify-between">
                                                <p className="text-xs">Tanggal {item.tanggal} | {item.penanggung_jawab}</p>
                                            ...
                                            <p className="text-xs">{item.klien}</p>
                                            <p className="text-xs w-max mt-2">{item.jumlah_dokumen} Dokumen Klien</p>

                                            <div className="flex justify-between">
                                                <div className="flex gap-x-2 bg-gray-500 text-white font-bold rounded-lg w-max mt-2">
                                                    <p className="text-xs w-max pl-2">{item.tipe_dokumen}</p>
                                                    <p className={`text-xs w-max border-l-2 px-2 border-white ${item.jenis_dokumen == "EMPTY" ? "" : "border-r-2"}`}>{item.kategori_dokumen}</p>
                                                    {
                                                        item.jenis_dokumen != "EMPTY" ? <>
                                                            <p className="text-xs w-max pr-2">{item.jenis_dokumen}</p>
                                                        </> : <></>
                                                    }
                                                </div>
                                                {
                                                    item.hutang == 0 ? <div className="flex gap-x-2 bg-green-700 text-white font-bold rounded-lg w-max mt-2">
                                                        <p className="text-xs px-2">Lunas</p>
                                                    </div> : <div className="flex gap-x-2 bg-red-500 text-white font-bold rounded-lg w-max mt-2">
                                                        <p className="text-xs px-2">Hutang Rp. {parseToRupiahText(item.hutang)}</p>
                                                    </div>
                                                }
                                            </div>

                                            {
                                                item.tanggal_riwayat_aktivitas_uuid ? <>
                                                    <div className="border-l-2 border-blue-800">
                                                        <p className="text-xs bg-blue-800 w-max px-3 rounded-r-lg text-white mt-4 font-bold">Aktivitas Terakhir</p>
                                                        <div className="pl-2">
                                                            <p className="text-[10px] font-bold mt-2">{item.tanggal_riwayat_aktivitas_uuid}</p>
                                                            <p className="text-xs mt-1 text-gray-700">{item.judul_riwayat_aktivitas_uuid}</p>
                                                            <p className="text-xs bg-blue-600 w-max px-3 rounded text-white mt-4 font-bold">Status Terakhir</p>
                                                            <p className="text-[10px] font-bold mt-2">{item.tanggal_status_riwayat_aktivitas_terakhir}</p>
                                                            <p className="text-xs mt-1 text-gray-700">{item.judul_status_riwayat_aktivitas_terakhir}</p>
                                                        </div>
                                                    </div>
                                                </> : <></>
                                            }
