...
export const getSumMinusOfStringValue = (data) => {
    try {
        const total = data.reduce((accumulator, currentValue, idx) => {
            // Mengonversi nilai dari string menjadi angka menggunakan parseFloat() atau parseInt()
            const angka = parseFloat(currentValue);
            // Mengurangkan angka dari nilai akumulator
            return idx > 0 ? accumulator - angka : angka;
        }, 0); // Nilai awal akumulator adalah 0
