...
import RincianPelunasanPembelianBarangModel from "./rincianPelunasanPembelianBarang.model.js";
import { generateDatabaseName, insertQueryUtil, selectOneQueryUtil, updateQueryUtil } from "../../utils/databaseUtil.js";
import { removeDotInRupiahInput } from "../../utils/numberParsingUtil.js";

export const getAllRincianPelunasanPembelianBarangRepo = async (pageNumber, size, search, req_id) => {
    const rincianPelunasanPembelianBarangsCount = await db.query(
        `
            SELECT COUNT(0) AS count FROM ${generateDatabaseName(req_id)}.rincian_pelunasan_pembelian_barang_tab WHERE pelunasan_pembelian_barang LIKE '%${search}%' AND enabled = 1
        `,
        { type: Sequelize.QueryTypes.SELECT }
    )

    pageNumber = pageNumber && pageNumber > -1 ? pageNumber : 0
    size = size ? size : rincianPelunasanPembelianBarangsCount[0].count

    const rincianPelunasanPembelianBarangs = await db.query(
        `
            SELECT * FROM ${generateDatabaseName(req_id)}.rincian_pelunasan_pembelian_barang_tab WHERE pelunasan_pembelian_barang LIKE '%${search}%' AND enabled = 1 LIMIT ${pageNumber}, ${size}
        `,
        { type: Sequelize.QueryTypes.SELECT }
    )

    return {
        entry: rincianPelunasanPembelianBarangs,
        count: rincianPelunasanPembelianBarangsCount[0].count,
        pageNumber: pageNumber == 0 ? pageNumber + 1 : (pageNumber / size) + 1,
        size
    }
}

export const getAllRincianPesananPembelianBarangByPelunasanPembelianRepo = async (uuid, tanggal, faktur_pembelian_barang, req_id) => {
            ...
            FROM (
                SELECT 
                    ...
                    (
                        ...
                        AND ppbt2.enabled = 1
                    ) AS rincian_pelunasan_pembelian_barang,
                    IFNULL((
                        SELECT 
...
export const getRincianPelunasanPembelianBarangByUuidRepo = async (uuid, req_id) => {
    return selectOneQueryUtil(
        generateDatabaseName(req_id),
        RincianPelunasanPembelianBarangModel,
        null,
        {
            uuid,
            enabled: true
...
export const createRincianPelunasanPembelianBarangRepo = async (rincianPelunasanPembelianBarangData, req_id) => {
    ...
    return insertQueryUtil(
        ...
        RincianPelunasanPembelianBarangModel,
        {
            pelunasan_pembelian_barang: rincianPelunasanPembelianBarangData.pelunasan_pembelian_barang,
            rincian_pesanan_pembelian_barang: rincianPelunasanPembelianBarangData.rincian_pesanan_pembelian_barang,
...
export const deleteRincianPelunasanPembelianBarangByUuidRepo = async (uuid, req_id) => {
    updateQueryUtil(
        req_id,
        generateDatabaseName(req_id),
        RincianPelunasanPembelianBarangModel,
