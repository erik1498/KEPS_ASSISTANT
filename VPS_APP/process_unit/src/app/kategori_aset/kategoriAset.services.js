...
import { checkKategoriAsetSudahDigunakanRepo, createKategoriAsetRepo, deleteKategoriAsetByUuidRepo, getAllKategoriAsetRepo, getKategoriAsetByUuidRepo, updateKategoriAsetByUuidRepo } from "./kategoriAset.repository.js"
...
export const deleteKategoriAsetByUuidService = async (uuid, req_identity) => {
    ...
    await checkKategoriAsetSudahDigunakanService(uuid, req_identity)

    await deleteKategoriAsetByUuidRepo(uuid, req_identity)
    return true
}

export const checkKategoriAsetSudahDigunakanService = async (uuid, req_identity) => {
    LOGGER(logType.INFO, `Start checkKategoriAsetSudahDigunakanService`, {
        uuid
    }, req_identity)
    const kategoriAsetGet = await checkKategoriAsetSudahDigunakanRepo(uuid, req_identity)
