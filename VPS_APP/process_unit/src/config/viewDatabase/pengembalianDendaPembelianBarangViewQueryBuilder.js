...
export const pengembalianDendaPembelianBarangViewQueryBuilder = (req_id) => {
    return `
        CREATE VIEW ${generateDatabaseName(req_id)}.pengembalian_denda_pembelian_barang_view AS
        SELECT
            ...
            "PENGEMBALIAN DENDA PEMBELIAN BARANG" AS sumber
...
export const getPengembalianDendaPembelianBarangViewQuery = (req_id) => {
    return `
        SELECT 
            COUNT(0) AS count
        FROM INFORMATION_SCHEMA.VIEWS
        WHERE TABLE_SCHEMA = '${generateDatabaseName(req_id)}' 
        AND TABLE_NAME = 'pengembalian_denda_pembelian_barang_view';
    `
}

export const getDataFrompengembalianDendaPembelianBarangViewQuery = (bulan, tahun, kode_akun_perkiraan, req_id) => {
    return `
        SELECT 
            pdpbv.* 
        FROM ${generateDatabaseName(req_id)}.pengembalian_denda_pembelian_barang_view pdpbv
