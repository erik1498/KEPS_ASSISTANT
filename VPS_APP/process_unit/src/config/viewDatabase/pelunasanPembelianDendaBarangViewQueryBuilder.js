...
export const pelunasanPembelianDendaBarangViewQueryBuilder = (req_id) => {
    return `
        CREATE VIEW ${generateDatabaseName(req_id)}.pelunasan_pembelian_denda_barang_view AS
            SELECT 
                ...
                0 AS transaksi,
                rppbt.nilai_pelunasan_denda AS debet,
                0 AS kredit,
            ...
            SELECT 
                ...
                1 AS transaksi,
                0 AS debet,
                rppbt.nilai_pelunasan_denda AS kredit,
                kapt.name AS nama_akun,
                kapt.code AS kode_akun,
                kapt.type AS type_akun,
                kapt.uuid AS uuid_akun,
...
export const getPelunasanPembelianDendaBarangViewQuery = (req_id) => {
    return `
        SELECT 
            COUNT(0) AS count
        FROM INFORMATION_SCHEMA.VIEWS
        WHERE TABLE_SCHEMA = '${generateDatabaseName(req_id)}' 
        AND TABLE_NAME = 'pelunasan_pembelian_denda_barang_view';
    `
}

export const getDataFromPelunasanPembelianDendaBarangViewQuery = (bulan, tahun, kode_akun_perkiraan, req_id) => {
    return `
        SELECT 
            ppdblv.* 
        FROM ${generateDatabaseName(req_id)}.pelunasan_pembelian_denda_barang_view ppdblv
        ${kode_akun_perkiraan ? `WHERE ppdblv.uuid_akun = "${kode_akun_perkiraan}"` : ``}
        ${kode_akun_perkiraan ? `
            AND ppdblv.bulan = ${bulan}
            AND ppdblv.tahun = ${tahun}
        ` : `
            WHERE ppdblv.bulan = ${bulan}
            AND ppdblv.tahun = ${tahun}
        `}
    `
}